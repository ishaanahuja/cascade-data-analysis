...
Since its introduction, the theory Perturbative Quantum Chromodynamics (PQCD) has made huge strides and its calculations for the strong coupling constant
...
The experimental endaevours in the search for a deconfined state of matter began as early as 1986 with fixed target experiments at the Alternating Gradient
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{RMS values of anistropic flow coefficients $\langle v_{n}^{2}\rangle^{1/2}$. Left plot from ALICE Collaboration depicts $\langle v_{n}^{2}\rangle^{1/2}$
        computed as a function of centrality and compared to experimental data based on two-particle correlations $v_{n}\{2\}, n\in 2,3,4$. Plot on the right from
        the PHENIX and STAR collaboration shows a comparison of $v_{n}(p_{\textnormal{\sc t}})$ at RHIC using a constant $\eta/s$ = 0.12 (top) and a temperature
...
Besides azimuthal correlations, pseudorapidity correlations originating in early phases of the collision are also observed. A significant correlation
between the variations in pseudorapidity and azimuthal angle between pairs of particles across several rapidity units was discovered in Au-Au collisions at
RHIC and was dubbed the "ridge"\cite{Abelev:2009}. A remarkably similar structure was observed in high multiplicity proton-proton collisions at the LHC by
CMS\cite{Khacha:2010,Khacha:2016} and ATLAS\cite{Aad:2016} experiments and in Pb-Pb collisions by ALICE\cite{Aamodt:2012} and CMS\cite{CMS:2012}
experiments (Fig. \ref{fig:CMS}).
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{images/cms.pdf}
    \\
    \caption{ Two-dimensional per-trigger-particle associated yield of charged hadrons for $3<p_{\textnormal{\sc t}}^{\textnormal{trig}}<3.5 \textnormal{GeV}/c$
        and $1<p_{\textnormal{\sc t}}^{\textnormal{assoc}}<1.5 \textnormal{GeV}/c$ as a function of $|\Delta\eta|$ and $|\Delta\phi|$ distributed across 12
        centrality ranges of Pb-Pb collisions at $\sqrt{s_{\textnormal{\sc nn}}} = 2.76$ TeV. In the two most peripheral distributions, the near-side peak is
        trimmed to highlight the surrounding structure\cite{CMS:2012}.}
    \label{fig:CMS}
\end{figure}
...
$R_{\textnormal{\sc aa}}$ measurements in p-Pb collisions at ALICE\cite{sami:2016} yield a value of unity consistent within systematic uncertainities across the
$p_{\textnormal{\sc t}}$ range, essentially diregarding any jet quenching and, therefore, QGP formation. Fig. \ref{fig:raappb} illustrates $R_{\textnormal{pPb}}$
...
One of the earliest suggested signatures for the formation of QGP was an increased yield of strange particles in heavy-ion collisions as compared to
proton-proton collisions\cite{Muller:1982}. In the event of a deconfined phase of quarks and gluons (QGP), strange quark pairs production ($s\bar{s}$) can
...
\begin{figure}[H]
    ...
    \\
    \caption{Particle yield ratios $\Lambda/K_{\textnormal{\sc s}^{0}}$ and $p/\pi$ as a function of 
