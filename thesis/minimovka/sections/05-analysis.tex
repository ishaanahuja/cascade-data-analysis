...
strange quark content makes them susceptible to changes in hadrochemistry of the colliding systems. Though smaller systems like p-p and p-Pb are not expected
to produce suitable conditions for QGP formation, nevertheless their study acts as a reference to judge the enhancement in heavy-ion collisions. Recent
results from smaller systems show an increasing trend of strangeness with increasing event multiplicity, adding a new aspect to one of the earliest
signatures of QGP formation. This analysis attempts to probe the multi-strange
...
$\overline{\Xi^{0}}$, $\overline{\Xi^{+}}$, and $\overline{\Omega^{+}}$ are of interest. They are known as cascade particles owing to their doubly weak
decay mechanism.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|r|l|r|r|}
        ...
        $\Lambda / \overline{\Lambda}$ & $1115.683 \pm 0.006$ & $p^{\pm} \pi^{\mp}$ & $63.9 \pm 0.05$      & 7.89                  \\ \hline
    \end{tabular}
    \caption{Selected hyperons and their properties\cite{Pdg:2018}}
    \label{tab:cascades}
\end{table}
...
charged cascade candidates $\Xi^{-}$, $\Omega^{-}$, $\overline{\Xi^{+}}$, and $\overline{\Omega^{+}}$. The analysis involves initial event, track and cascade
...
and corrected spectra, and evaluating systematic uncertainties.
...
where $p$ is the total momentum, $\tau$ refers to the proper lifetime and $m$ is mass of the particle. $\Xi^{\pm}$ ($\Omega^{\pm}$)\footnote{the
    notations $\Xi^{\pm}$ and $\Omega^{\pm}$ stand for: $\Xi^{\pm}$ = $\overline{\Xi^{+}}$ + $\Xi^{-}$; $\Omega^{\pm}$ = $\overline{\Omega^{+}}$ + $\Omega^{-}$} primarily decay into a
...
relatively low interaction rates (<50 kHz) indicating low out-of-bunch event pileup. These runs represent about 40\% of the total statistics available for this
period. The full processed statistics for the analysis comprises of $15.7 \times 10^{6}$ events.

The data sample is available as AOD\cite{alirootAOD} files on CERN's computing network. The analysis code is written in CERN's software framework for data analysis: ROOT 6\cite{rootTutorial},
in conjunction with AliRoot\cite{aliRoot} libraries from the ALICE team. The analysis was performed on CERN's GRID network.

\section{Event selection criteria}

Basic event selection criteria following ALICE convention for p-Pb collisions\cite{aliceCuts} were applied in this analysis. The event selection performed include $\colon$
...
\begin{itemize}
    ...
    \item \textbf{Event multiplicity selection}: Due to the asymmetric nature of the collision, the multiplicity estimator based on the VZERO-A amplitude was
          selected (Pb-going direction).
\end{itemize}

Moreover, event multiplicity classes to maximise the multiplicity bins that could be explored with the present statistics were chosen. The classes are
...
\begin{itemize}
    ...
    \item \textbf{TPC d\textit{E}/{d}\textit{x} selection}: A cut on TPC's energy loss d\textit{E}/{d}\textit{x}
...
All selection criteria involved in this analysis are listed in Table \ref{tab:crit}. For better understanding, the topological variables are illustrated
in fig. \ref{fig:cascDecay} and fig. \ref{fig:topo}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|r|}
        ...
        DCA\tablefootnote{DCA = Distance of Closest Approach} (bachelor - PV)                & >0.04 cm                                \\ \hline
        DCA (V0 - PV\tablefootnote{PV = Primary Vertex})                                     & >0.06 cm                                \\ \hline
        ...
        TPC\tablefootnote{TPC = Time Projection Chamber} d\textit{E}/{d}\textit{x} selection & < 4$\sigma$                             \\ \hline
...
\begin{figure}[H]
    ...
    \caption{Plots showing examples of invariant mass for $\Xi^{+}$ (top) and $\Omega^{-}$ (bottom). $y$-axis denotes Counts/(2 MeV/$c^{2}$).}
    \label{fig:sigeg}
\end{figure}
