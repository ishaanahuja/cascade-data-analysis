\chapter{Overview of the experimental setup} \label{sec:LHC}
...
with LHC being the final accelerator currently boasting energies up to 6.8 TeV per beam for Run 3. The analysis presented in this work uses data 
from LHC's Run 2, where the beams were accelerated up to 6.5 TeV.
...
occur at four chosen intersection points. In LHC's Run 2 data taking period, the collisions could amount to as much as 13 TeV centre-of-mass energy and 
peak luminosity values of $2\times10^{34}cm^{-2}s^{-1}$\cite{LhcReport:2018}. These collisions are studied by several experimental stations situated 
throughout the beamline. Four main experiments in the LHC complex, employing massive detector assemblies, include:
...
ranges. The data presented in this report are based on p-Pb collisions at centre-of-mass energy of 8.16 TeV observed at the ALICE detector.
...
\begin{enumerate}
    ...
    \item High Momentum Particle Identification (HMPID)
\end{enumerate}
...
\begin{itemize}
    ...
    \item Tracking and identification of particles with momentum < 100 MeV$/c$.
    \item Momentum and angle resolution improvements for high $p_{\textnormal{\sc t}}$ particles traversing through the TPC.
...
The Time Projection Chamber (TPC)\cite{TPC:2010} is the main particle identification and tracking detector in the ALICE experiment. TPC's strengths include charged
...
TPC's axis is 
aligned with the LHC's beamline, and it operates in a 0.5 T solenoidal magnetic field parallel to the ALICE detector's magnetic field. Cylindrical in 
shape with an inner radius of 848 mm and an outer radius of 2466 mm, the TPC is filled with almost 90 m$^{3}$ of a gaseous mixture of 
Ne-CO$_{2}$-N$_{2}$
...
The Time-of-Flight (TOF) detector\cite{TOF:2000}, with the support of TPC, is used for identifying charged particles by determining their mass. 
...
Similar to the TPC, TOF also follows a hollow cylinder design with an outer radius of 3.78 m and 18-fold segmentation. Covering the full azimuthal range 
and $|{\eta}| < 0.9$, TOF is able to provide an average time resolution better than 50 ps\cite{TOF:2019}, enabling pile-up corrections through separating 
particles produced in interactions of different bunch crossings. 
