...
and consitituent forces were reached. The developments in Quantum Chromodynamics shed light on another layer of matter lurking inside the 
...
quarks have not yet been identified to exist freely except in very special conditions involving extreme energy densities. These extreme 
...
such that precise assessments can be made about the collsion epoch over a high range of multiplicities and momenta.
...
$\Xi^{\pm}$ and $\Omega^{\pm}$ in p-Pb collisions at $\sqrt{s_{NN}}$ = 8.16 TeV during LHC's Run 2 data taking period are studied. Finally, 
current results showing the raw $p_{T}$ spectra are presented.


\section{The Standard Model}
The Standard Model\cite{SM:2018,Griffiths:2008}, a culmination of numerous theories and discoveries towards an insight into the fundamental 
...
can be attributed to the four fundamnental forces(Fig. \ref{fig:4forces}), or a combination thereof: 
\begin{figure}[H]
    \centering
...
weak interactions with a comparative strength, and clubbed together, they are depicted by the unified theory of electroweak interactions. The theory 
...
forces, it can be omitted at present in the standard model. However, on a macroscopic scale, gravitational interaction holds a much more 
...
in nature with a spin 0 value, is a recent addition to the standard model. This particle, known as the Higgs boson, is deemed to be responsible for 
assigning mass to particles. Bosons follow the Bose-Einstein statistics for the formation of composite particles.
...
exclusion principle. As an example, the discovery of delta baryon --  $\Delta^{++}$ (u,u,u) consisiting of three up quarks with parallel spin could 
be validated. 
...
colour-singlets) below the Hagedorn temperature ($\sim10^{12} K)$. This phenomenon is termed as colour confinement\cite{Florkowski:2010} and is 
...
quarks. This effect gives rise to a phenomenon known as asymptotic freedom\cite{Gross:1973}, indicating that larger distances between interacting 
...
$\alpha_{s}$, which can be defined as follows\cite{Florkowski:2010}:

\medskip
\begin{equation}
  \alpha_{s}(Q^{2})=\frac{16\pi^{2}}{(11 - \frac{2}{3}N_{f})ln(\frac{Q^{2}}{\Lambda_{QCD}^{2}})} \hspace{0.15cm} \xrightarrow{Q^{2}\to\infty} 0,
\label{eq:CouplingQCD}
\end{equation}
\smallskip

where Q is the Lorentz invariant generalization of momentum transfer, $N_{f}$ is the number of included quark flavours that can partake in the 
screening loop and $\Lambda_{QCD}$ is the QCD scale parameter determined either experimentally or from lattice QCD calculations such that the 
coupling constant is calibrated at mass of $Z^{0}$ boson ($\Lambda_{QCD} \approx 200MeV)$. From the experimental results (Fig. \ref{fig:CouplingQCD}), 
...
This occurrence points to the creation of a virtually non-interacting QGP at extreme temperatures\cite{Collins:1975}.
...
has been hypothesized early on and presumed to exist in neutron stars and the early universe, shortly after the Plank epoch in the Big Bang theory. 
Attempts to recreate this primordial liquid have been carried out by various particle accelerators and collider experiments in order to achieve 
immense pressure values to 'merge' the hadrons together and create high enough temperatures to excite quarks out of their bound state in hadrons. 
The extreme temperatures ($\sim 1.97\times10^{12}K$) and particle density should result in the merging of confined partons to an incredibly hot, 
...
to provide sufficient conditions for the creation of this new state of matter.
...
[[The following section is somewhat incomplete, will be improved/extended.]]

The grand canonical ensemble characterizes nuclear matter in thermodynamic equilibrium with respect to temperature $T$, the baryo-chemical 
potential $\mu_{B}$, and volume $V$. Here, $\mu_{B}$ is the chemical potential of the conserved baryon number. Figure \ref{fig:QcdPhaseDiagram} 
depicts a schematic representation of the phase diagram of nuclear matter in the $\mu_{B}, T$ plane. Nuclear matter, ordinarily, exists at 
$T\approx0$ and $\mu_{B}\approx1 GeV$. The phase boundary ending in a critical end-point differentiates the hadron resonance gas phase from the 
quark-gluon plasma. The critical point location has also been determined from lattice QCD approach\cite{Fodor:2002}. At minimal $\mu_{B}$ values, 
the critical temperature for transition's cross-over is ($T_{c}\approx170 MeV$). On the other hand, at low values of temperature but baryon density 
exceeding a few times of nuclear matter density, a deconfined color superconducting phase is predicted, possible along with other phases\cite{Fukushima:2013}.

Lattice calculations have yielded a somewhat lower transition critical temperature ($T_{c}\approx144 MeV$), with the critical net 
baryon density ($\mu_{u,d}=434 MeV$)\cite{Wong:1994} in agreement with chemical freeze-out temperature from statistical hadronization model. 

\begin{figure}
  \centering
  \includegraphics[width=0.8\linewidth]{images/QcdPhaseDiagram.pdf}
  \caption{\small The phase diagram of QCD: solid lines indicate first-order transitions, dashed lines indicating possible continuous rapid crossover transition\cite{Carminati:2004}.}
  \label{fig:QcdPhaseDiagram}
\end{figure}


The QGP state, by itself, is quite unstable owing to the extreme conditions required for its creation and maintainence, hence it exists for a 
...
estimated to be 4 to 6 fm/c\cite{Sahoo:2010}, hence any observation for its generation are required to be probed in this interval or at the 
earliest collision moment so as to avoid any subsequent hadronization. This presents a challenge to make direct observations for QGP's existence 
...
\begin{enumerate}
  ...
  \item Increased yield of strange and multi-strange mesons
  \item Jet Quenching
