...
int RunVariationsFit(
    TString inputFilename = "/home/alice/iahuja/RandomVars/130125_LHC17l7a2_cent_RandomVars.root",
    TString outputFolder = "/home/alice/iahuja/RandomVars/140125_McRand_LHC17l7a2",
    TString outputFilePrefix = "140125_McRand_LHC17l7a2",
    bool fisMC = true,
    bool fitDiffForVars = false,
    ...
    Int_t verbosity = kInfo)
{
  ...
  /// VARIATIONS:
  for (Int_t iVar = 393; iVar < nVar; iVar++)
  {
    TDatime now;
    if (sleepCounter == 90)
    {
      Info("RunVariationsFit_Vars", "%s: \e[1;94mCatching Breath: Pausing execution for 10 seconds.\e[0m", now.AsSQLString());
      gSystem->Sleep(10000); // sleep for 10s
      sleepCounter = 0;
    }
