...
// Include your utility headers/macros
...
// Helper function to combine stat & sys errors from YieldMean output TH1
// Calculates total error as sqrt(stat^2 + avg_sys^2) where avg_sys = (sysHi+sysLo)/2
// Returns asymmetric errors as well (stat, sysLow, sysHigh)
// void GetCombinedErrors(TH1 *hYieldInfo, EValue_t valBin, EValue_t statBin, EValue_t sysLoBin, EValue_t sysHiBin,
//                        Double_t &val, Double_t &totalError, Double_t &statError, Double_t &sysErrorLow, Double_t &sysErrorHigh)
// {
//     if (!hYieldInfo)
//     {
//         val = 0;
...
//     // Use average symmetric systematic error for TGraphErrors compatibility
//     Double_t avgSysError = (sysErrorLow + sysErrorHigh) / 2.0;
//     totalError = TMath::Sqrt(statError * statError + avgSysError * avgSysError);
// }
    ...
    TString imageFormat = "pdf")
{
    ...
    // Particle definitions (matching RunYieldMean)
    // const int nParticlesPlot = kNumSignedPart; // kXip, kXim, kOmp, kOmm, kXiC, kOmC
    // const char *particleNamePlot[nParticlesPlot] = {"xip", "xim", "omp", "omm", "xiC", "omC"};
    // const char *particleTitlePlot[nParticlesPlot] = {"#Xi^{+}", "#Xi^{-}", "#Omega^{+}", "#Omega^{-}", "#Xi^{+} + #Xi^{-}", "#Omega^{+} + #Omega^{-}"};
    // const char *particleDirName[nParticlesPlot] = {"YieldResults_xip", "YieldResults_xim", "YieldResults_omp", "YieldResults_omm", "YieldResults_xiC", "YieldResults_omC"}; // Dir names in results file
    // const double *dNch[nParticlesPlot] = {dNchXi, dNchXi, dNchOm, dNchOm, dNchXi, dNchOm};
    // const double *dNchErr[nParticlesPlot] = {dNchXiErr, dNchXiErr, dNchOmErr, dNchOmErr, dNchXiErr, dNchOmErr};
    // const int *nMultBinsPlot[nParticlesPlot] = {&fNmultbins_Xi, &fNmultbins_Xi, &fNmultbins_Om, &fNmultbins_Om, &fNmultbins_Xi, &fNmultbins_Om};
    // const double *multBinEdgesPlot[nParticlesPlot] = {&fMultbins_Xi[0], &fMultbins_Xi[0], &fMultbins_Om[0], &fMultbins_Om[0], &fMultbins_Xi[0], &fMultbins_Om[0]};
    ...
    // ** MODIFICATION START: Populate yieldsPerFuncAll and meansPerFuncAll first **
    Info("PlottingUtils", "Collecting results from all fit functions for extrapolation uncertainty...");
    for (int iPart = 0; iPart < nParticlesPlot; ++iPart)
    {
        ...
        for (int iMult = 0; iMult < nMult; ++iMult)
        {
            for (int iFunc = 0; iFunc < kNumFitFunctions; ++iFunc)
            {
                ...
                else
                {
                    ...
                    Warning("PlottingUtils", "YieldInfo not found (will skip for extrap): %s", yieldInfoNameFunc.Data());
                }
            }
        }
    }
    ...
    // ** MODIFICATION START: Calculate and store extrapolation systematic uncertainties **
    ...
    Info("PlottingUtils", "Calculating extrapolation systematic uncertainties...");
    for (int iPart = 0; iPart < nParticlesPlot; ++iPart)
    {
        int nMult = *nMultBinsPlot[iPart];
        for (int iMult = 0; iMult < nMult; ++iMult)
        {
            ...
            { // Check if Levy result was valid
                Warning("PlottingUtils", "Levy-Tsallis result not found or invalid for %s mult %d. Skipping extrap. uncertainty.", particleNamePlot[iPart], iMult);
                continue;
            }
    ...
    // --- CORRECTED SECTION: Output Final Values to Console in a Formatted Table ---
    Info("PlottingUtils", "--- Generating Summary Tables for Final Results ---");
    ...
    // --- Generate Yield vs Multiplicity (Fig 5.7) ---
    // This part remains the same as before, it reads from the main results file
    ...
    for (int iPart = 0; iPart < nParticlesPlot; ++iPart)
    {
        ...
        for (int iMult = 0; iMult < nMult; ++iMult)
        {
            ...
            if (!hYieldInfo)
            {
                Warning("PlottingUtils", "YieldInfo histogram not found for Yield plot: %s", yieldInfoName.Data());
                continue;
            }
            ...
            // ** MODIFICATION START: Combine sys errors for Yield **
            ...
            // ** MODIFICATION START: Store separate errors **
            v_YieldStatErr.push_back(statErrVal);
            // ** MODIFICATION START: Store combined sys errors **
            ...
            //         Warning("PlottingUtils", "Missing YieldInfo for extrap uncertainty: %s", yieldInfoNameFunc.Data());
            //     }
            // }
        ...
        // ** MODIFICATION: Use combined sys errors for TGraphAsymmErrors **
        ...
        // ... [Rest of styling and drawing for Yield plot is the same as before] ...
        ...
        // ** MODIFICATION START: Draw sys boxes first, then stat errors/points **
        if (gYieldSys[iPart])
    ...
    // --- Generate Yield vs Multiplicity (Fig 5.7 style) with Linear Fits ---
    TCanvas *cYieldWithFit = new TCanvas("cYieldVsMultWithFit", "Yield vs dNch/deta with Linear Fit", 1200, 800);
    // gYieldStat and gYieldSys are already declared from the user's PlottingUtils.C file structure
    // TF1 *linearYieldFits[nParticlesPlot] = {nullptr}; // Already declared at the top of the function

    for (int iPart = 0; iPart < nParticlesPlot; ++iPart)
    {
        cYieldWithFit->Clear(); // Clear canvas for each particle plot

        // Vectors for data points (assuming v_dNch, v_Yield etc. are populated as in your original script)
        // For this example, I'll re-fetch from gYieldStat[iPart] if necessary, but your existing
        // population of gYieldStat and gYieldSys should work directly.
        // The following lines assume gYieldStat[iPart] and gYieldSys[iPart] have been correctly
        // populated earlier in your script for the current iPart, as per your original logic.
        // If not, ensure they are populated before this point in your actual script.
        // This revised section will use the already populated gYieldStat[iPart] and gYieldSys[iPart].

        if (!gYieldStat[iPart] || !gYieldSys[iPart] || gYieldStat[iPart]->GetN() == 0)
        {
            Warning("PlottingUtils", "gYieldStat/Sys for particle %d not populated. Skipping yield plot with fit.", iPart);
            continue;
        }

        // Dynamic frame calculation (similar to your original logic, slightly adapted)
    ...
    // ... [Code for individual yield plots with fits, and optionally comparison plots] ...

    // --- NEW: Plot ratio of linear yield fits (Omega/Xi) ---
    Info("PlottingUtils", "Generating ratio of linear yield fits (Omega/Xi)...");
    // Ensure that particleNamePlot[0] is Xi and particleNamePlot[1] is Omega as per your setup
    if (nParticlesPlot >= 2 && linearYieldFits[0] && linearYieldFits[1])
    {
        ...
        {
            // Warning("PlottingUtils", "Common dNch range for fit ratio is ill-defined [%f, %f], using defaults [0, 55]", dNch_min_common, dNch_max_common);
            dNch_min_common = 0;
            dNch_max_common = 55;
            Warning("PlottingUtils", "Common dNch range for fit ratio is ill-defined, using default [%f, %f]", dNch_min_common, dNch_max_common);
        }
        ...
        { // Fallback if GetMinimum/Maximum fails or returns NaN
            ...
            Warning("PlottingUtils", "Could not determine Y-range for ratio plot dynamically, using default [%f, %f]", y_min_ratio, y_max_ratio);
        }
    ...
    {
        Warning("PlottingUtils", "Linear yield fits for Omega and/or Xi not available. Skipping ratio plot.");
        if (nParticlesPlot < 2)
        {
            Warning("PlottingUtils", "nParticlesPlot is %d, need at least 2 for Omega/Xi ratio.", nParticlesPlot);
        }
        if (!linearYieldFits[0])
            Warning("PlottingUtils", "Fit for %s (index 0) is missing.", particleNamePlot[0]);
        if (nParticlesPlot >= 2 && !linearYieldFits[1])
            Warning("PlottingUtils", "Fit for %s (index 1) is missing.", particleNamePlot[1]);
    }
    // --- END OF RATIO PLOT SECTION ---

    // --- FINAL SECTION: Generate Yield vs Multiplicity plots with all data sources ---
    Info("PlottingUtils", "Generating Yield vs. Multiplicity plots with extensive data comparison from pp, p-Pb, and Pb-Pb...");
    ...
    Info("PlottingUtils", "Generating Yield vs. Multiplicity plots with Table 3 Data Comparison...");
    ...
    // --- END OF NEW SECTION ---
    // --- Generate Mean pT vs Multiplicity (Fig 5.8) ---
    // This part also remains the same as before
    ...
    for (int iPart = 0; iPart < nParticlesPlot; ++iPart)
    {
        ...
        for (int iMult = 0; iMult < nMult; ++iMult)
        {
            ...
            // ** MODIFICATION START: Combine sys errors for Mean pT **
        ...
        // ... [Rest of styling and drawing for Mean Pt plot is the same as before] ...
    ...
    // --- Generate Spectra Fit Plots (Fig 5.5, 5.6 style) for ALL bins ---
    // --- Using fits retrieved from log files ---
    Info("PlottingUtils", "Generating spectra fit plots for all multiplicity bins...");
    ...
    for (int iPart = 0; iPart < nParticlesPlot; ++iPart)
    {
        ...
        for (int iMult = 0; iMult < nMult; ++iMult) // Loop through ALL multiplicity bins
        {
            ...
            Info("PlottingUtils", "  Plotting spectra for %s, mult %.0f-%.0f %%", particleNamePlot[iPart], multLow, multHigh);
            ...
            if (!hStat || !hSys)
            {
                Error("PlottingUtils", "Spectra hist not found for %s mult %d. Skipping plot.", particleNamePlot[iPart], iMult);
                delete cSpecFit;
                continue;
            }
            ...
            if (nDataPoints == 0)
            {
                Error("PlottingUtils", "No data points in %s", histNameStat.Data());
                delete cSpecFit;
                continue;
            }
            ...
            // ** MODIFICATION START: Draw frame with new pT range, then boxes, then stat points **
            ...
            legSpecFit->SetBorderSize(0);
            legSpecFit->SetFillStyle(0);
            // ** MODIFICATION: Add entry for combined data (stat+sys visually) **
            if (gDataPointsStat)
            ...
            for (int iFunc = 0; iFunc < kNumFitFunctions; ++iFunc)
            {
                ...
                {
                    Warning("PlottingUtils", "Could not open log file: %s", logFileName.Data());
                    if (logFile)
                        delete logFile;
                    continue;
                }
                ...
                if (hLogged)
                {
                    ...
                    // Info("PlottingUtils", "Found histogram '%s' in log file '%s'", loggedHistName.Data(), logFileName.Data());
                    fFitted = hLogged->GetFunction(Form("%s_%s_Mult%d", funcName.Data(), particleNamePlot[iPart], iMult));
                }
                else
                {
                    Warning("PlottingUtils", "Histogram '%s' not found in log file '%s'", loggedHistName.Data(), logFileName.Data());
                }
                ...
                if (fFitted)
                {
                    ...
                    // ** MODIFICATION: Ensure fit function is drawn across the new extended range **
                    ...
                    // Info("PlottingUtils", "Fit function '%s' found for histogram '%s' in log file '%s'", funcName.Data(), loggedHistName.Data(), logFileName.Data());
                }
                else
                {
                    Warning("PlottingUtils", "Fit function '%s' not found for histogram '%s' in log file '%s'", funcName.Data(), loggedHistName.Data(), logFileName.Data());
                }
                ...
                pad2->cd();
                {
                    ...
                    // ** MODIFICATION: Use temporary TH1 from original hStat for correct bin definitions for ratio **
                    TH1D *hAxisDefForRatio = (TH1D *)hStat->Clone("hAxisDefForRatio");
                    hAxisDefForRatio->Reset(); // Keep axes, clear content
                    // ** MODIFICATION: Use CreateRatioPlotFromGraph **
                    ...
                    if (hRatio)
                    {
                        Info("PlottingUtils", "Creating ratio plot for %s, mult %d, fit %s", particleNamePlot[iPart], iMult, funcName.Data());
                        // ** MODIFICATION START: Set X-axis range for ratio plot **
            ...
            // // else
            // // {
            // //     Warning("PlottingUtils", "YieldInfo histogram not found for Extrapolation fraction: %s", yieldInfoName.Data());
            // // }
            ...
            //     // Info("PlottingUtils", "Creating ratio plot for %s, mult %d", particleNamePlot[iPart], iMult);
            //     hRatio = CreateRatioPlot(hDataToPlot, levyFitFunc);
            //     // Info("PlottingUtils", "Ratio plot created for %s, mult %d", particleNamePlot[iPart], iMult);
            //     if (hRatio)
            //     {
            ...
            //         hRatio->Draw("PE");
            //         // Info("PlottingUtils", "Ratio plot drawn for %s, mult %d", particleNamePlot[iPart], iMult);
            ...
            //         line->SetLineColor(kGray + 2);
            //         line->Draw("same");
            //         // Info("PlottingUtils", "Reference line drawn for %s, mult %d", particleNamePlot[iPart], iMult);
            //     }
            // }
            // else
            // {
            //     Warning("PlottingUtils", "Levy-Tsallis fit was not found for ratio plot (%s, mult %d).", particleNamePlot[iPart], iMult);
            // }
            ...
            // pad2->cd();
            // TH1 *hRatio = nullptr;
            // if (levyFitFunc && gDataPointsStat)
            // {
            //     // ** MODIFICATION: Use temporary TH1 from original hStat for correct bin definitions for ratio **
            //     TH1D *hAxisDefForRatio = (TH1D *)hStat->Clone("hAxisDefForRatio");
            //     hAxisDefForRatio->Reset(); // Keep axes, clear content
            //     // ** MODIFICATION: Use CreateRatioPlotFromGraph **
            //     hRatio = CreateRatioPlotFromGraph(gDataPointsStat, levyFitFunc, hAxisDefForRatio);
            //     delete hAxisDefForRatio;
            //     // ** MODIFICATION END **
            ...
            //         // hRatio->GetYaxis()->SetTitle("Data / Levy-Tsallis Fit"); // Title for ratio y-axis
            //         hRatio->GetXaxis()->SetTitle("#it{p}_{T} (GeV/#it{c})"); // Add title to ratio x-axis
            //         hRatio->Draw("PE");                                      // Draw points on ratio
            //         // TLine *line = new TLine(pT_min_draw, 1, pT_max_draw, 1); // Line at 1
            //         // line->SetLineStyle(kDashed);
            //         // line->SetLineColor(kGray + 2);
            //         // line->Draw("same");
            ...
            // else
            // {
            //     Warning("PlottingUtils", "Levy-Tsallis fit or data graph not found for ratio (%s, mult %d).", particleNamePlot[iPart], iMult);
            // }
            ...
            // Info("PlottingUtils", "Saving spectra fit plot for %s, mult %d", particleNamePlot[iPart], iMult);
            ...
            // Info("PlottingUtils", "Saved spectra fit plot for %s, mult %d", particleNamePlot[iPart], iMult);
    ...
    Info("PlottingUtils", "Plots saved in: %s", outputPlotFolder.Data());
    Info("PlottingUtils", "Log files were read from: %s", inputLogFolder.Data());
}