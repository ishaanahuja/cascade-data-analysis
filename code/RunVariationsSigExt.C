...
/**
 * @brief Runs systematic variations for signal extraction for real and MC data
 *
 * This function processes both real and MC data to perform signal extraction fits
 * using the FitCascades macro. It handles multiple MC production runs and real data,
 * keeping track of execution times.
 *
 * @param outputFolder Path where output ROOT files will be saved
 * @param outputFilePrefix Prefix for output filenames
 * @param histSuffix Suffix for histogram identification (e.g. "DGP2_def")
 * @param fitFunction Integer specifying the fitting function to use (e.g. kDoubleGausPol2)
 * @param fitReal Boolean to control whether to process real data
 * @param verbosity Integer controlling the verbosity level of ROOT messages
 *
 * @return Returns 0 on successful completion
 *
 * The function:
 * - Enables multi-threading via ROOT::EnableImplicitMT()
 * - Processes real data if fitReal is true
 * - Processes 5 different MC productions
 * - Uses FitCascades.C macro for the actual fitting
 * - Tracks and reports execution times for each run
 * - Creates output files in the specified directory with naming convention:
 *   outputFilePrefix_histSuffix_[runIdentifier].root
 */
