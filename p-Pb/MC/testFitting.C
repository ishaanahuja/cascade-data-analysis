                ...
                TString outputFilename = "/var/home/ishaan/Work/git/analysis/p-Pb/MC/rootResults/DPMJET_GP_LHC17f3b/testFitting_all_noComp.root", TString idAxis = "ye", bool fisMC = false, bool bgTSpectrum = false, bool xi = true, bool om = true)
{
    ...
    if (xi)
    {
        ...
        resultParams_Xip_allInt = fitResults(nullptr, peakFnc_XiP, h_MassXip, fitMinSig_Xi, fitMaxSig_Xi, &par_allintP[0], &par_allint_errorsP[0], binsMC, MCGenXip, fisMC, kTRUE);
        peakFnc_XiM = setFitParameters(peakFnc_XiM, h_MassXim, lMass_Xi, -1, kTRUE);
        resultParams_Xim_allInt = fitResults(nullptr, peakFnc_XiM, h_MassXim, fitMinSig_Xi, fitMaxSig_Xi, &par_allintM[0], &par_allint_errorsM[0], binsMC, MCGenXim, fisMC, kTRUE);
        peakFnc_XiC = setFitParameters(peakFnc_XiC, h_MassXiC, lMass_Xi, -1, kTRUE);
        resultParams_XiC_allInt = fitResults(nullptr, peakFnc_XiC, h_MassXiC, fitMinSig_Xi, fitMaxSig_Xi, &par_allintC[0], &par_allint_errorsC[0], binsMC, MCGenXiC, fisMC, kTRUE);
    ...
    if (om)
    {
        ...
        resultParams_Omp_allInt = fitResults(nullptr, peakFnc_OmP, h_MassOmp, fitMinSig_Om, fitMaxSig_Om, &par_allintP[0], &par_allint_errorsP[0], binsMC, MCGenOmp, fisMC, kTRUE);
        peakFnc_OmM = setFitParameters(peakFnc_OmM, h_MassOmm, lMass_Om, -1, kTRUE);
        resultParams_Omm_allInt = fitResults(nullptr, peakFnc_OmM, h_MassOmm, fitMinSig_Om, fitMaxSig_Om, &par_allintM[0], &par_allint_errorsM[0], binsMC, MCGenOmm, fisMC, kTRUE);
        peakFnc_OmC = setFitParameters(peakFnc_OmC, h_MassOmC, lMass_Om, -1, kTRUE);
        resultParams_OmC_allInt = fitResults(nullptr, peakFnc_OmC, h_MassOmC, fitMinSig_Om, fitMaxSig_Om, &par_allintC[0], &par_allint_errorsC[0], binsMC, MCGenOmC, fisMC, kTRUE);
        ...
        for (Int_t ptBinOm = 0; ptBinOm < nptbins_Om; ptBinOm++)
        {
            ...
            for (Int_t multBinOm = 0; multBinOm < nmultbins_Om; multBinOm++)
            {
                ...
                // h_MassOmC_pt_mult[ptBinOm][multBinOm]->Write();
                // resultParOmC_pt_mult[ptBinOm][multBinOm]->Write();
                // TSpectrum *s_omC = new TSpectrum(1);
                ...
                //     s_omC->Background(source, nbinsX, 20, TSpectrum::kBackDecreasingWindow, TSpectrum::kBackOrder6, kTRUE, TSpectrum::kBackSmoothing15, kFALSE);
                ...
                // h_bgOmC_pt_mult[ptBinOm][multBinOm]->SetLineColor(kOrange);
                // for (int itest = 0; itest < nbinsX; itest++)
                //     h_bgOmC_pt_mult[ptBinOm][multBinOm]->SetBinContent(itest + 1, source[itest]);
                // // peakFnc_OmC = setFitParameters(peakFnc_OmC, h_MassOmC_pt_mult[ptBinOm][multBinOm], lMass_Om, ptBinOm + 1, kFALSE);
                // resultParOmC_pt_mult[ptBinOm][multBinOm] = fitResults(h_bgOmC_pt_mult[ptBinOm][multBinOm], peakFnc_OmC, h_MassOmC_pt_mult[ptBinOm][multBinOm], fitMinSig_Om, fitMaxSig_Om, &par_allintC[0], &par_allint_errorsC[0], binsMC, MCGenOmC, fisMC, kFALSE);
...
TH1 *fitResults(TH1 *h_bg, TF1 *peakFnc, TH1 *peak, Double_t fitMinSig, Double_t fitMaxSig, Double_t *par_allint, Double_t *par_allint_errors, Int_t binsMC[], TH2 *MCgen, bool fisMC, Bool_t allInt, Bool_t options)
{
                         ...
                         Double_t &err, Double_t eps = 1e-6)
{
    ...
    if (!fisMC)
    {
        ...
        if (h_bg == nullptr)
        {
            ...
            bgVal = bgFnc->Integral(minInt, maxInt, eps);

            // TODO Verify it
            bgErr = bgFnc->IntegralError(minInt, maxInt, fFitResult->GetParams(),
